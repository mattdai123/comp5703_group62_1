...
class PetrRefineTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self, query, *args, reference_points=None, reg_branches=None, joint_embeddings=None,
                non_joint_embeddings=None, joint_labels=None, **kwargs):
        output = query
        intermediate = []
        intermediate_reference_points = []
        contrastive_loss = torch.tensor(0.0, device=query.device)  # 初始化对比损失
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                ...
                new_reference_points[..., :3] = tmp[..., :3] + reference_points
                reference_points = new_reference_points.detach()

            output = output.permute(1, 0, 2)

            # 仅在最后一层计算对比损失
            if lid == len(self.layers) - 1 and joint_embeddings is not None and non_joint_embeddings is not None:
                contrastive_loss = self.compute_contrastive_loss(joint_embeddings, non_joint_embeddings, joint_labels)
    ...
    def compute_contrastive_loss(self, joint_embeddings, non_joint_embeddings, joint_labels):
        """
        ...
        Args:
            ...
            joint_labels (Tensor): Labels indicating joint types.
        ...
        # Loop over all pairs of joints to compute joint-to-joint distances
        ...
        # Joint-to-Non-Joint Comparisons (previous logic)
        joint_to_non_joint_distances = F.pairwise_distance(joint_embeddings[:, None, :],
                                                           non_joint_embeddings[None, :, :])

        # Contrastive loss calculation
        positive_loss = torch.mean(positive_distances ** 2)  # Minimize positive distances (same joints)
        negative_loss = torch.mean(torch.clamp(self.contrastive_margin - negative_distances,
                                               min=0) ** 2)  # Maximize negative distances (different joints)
        joint_to_non_joint_loss = torch.mean(torch.clamp(self.contrastive_margin - joint_to_non_joint_distances,
                                                         min=0) ** 2)  # Maximize joint-to-non-joint distances

        return positive_loss + negative_loss + joint_to_non_joint_loss
...
class PETRTransformer(Transformer):
    ...
    # learnable query and query_pos
        ...
        # 调用 refine_decoder 并接受返回的对比损失
        inter_states, inter_references, contrastive_loss = self.refine_decoder(
            query=query,
            key=pos_memory,
            value=pos_memory,
            query_pos=query_pos,
