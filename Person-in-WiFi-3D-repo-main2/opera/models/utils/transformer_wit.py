...
#         return output, reference_points
class PetrRefineTransformerDecoder(TransformerLayerSequence):
    """Implements the decoder in DETR transformer with added contrastive learning.
    ...
    def __init__(self, *args, return_intermediate=False, contrastive_margin=1.0, **kwargs):
        super(PetrRefineTransformerDecoder, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
        # Margin for contrastive loss
        self.contrastive_margin = contrastive_margin  

    def forward(self, query, *args, reference_points=None, reg_branches=None, joint_embeddings=None, non_joint_embeddings=None, **kwargs):
        """
        Forward function for Transformer Decoder with contrastive learning.

        Args:
            query (Tensor): Input query with shape `(num_query, bs, embed_dims)`.
            reference_points (Tensor): The reference points for keypoint offset, has shape `(bs, num_query, 4)`.
            joint_embeddings (Tensor): Positive samples (joint embeddings) for contrastive learning.
            non_joint_embeddings (Tensor): Negative samples (non-joint embeddings) for contrastive learning.
        ...
        Returns:
            ...
            contrastive_loss (Tensor): Computed contrastive loss for joint-to-non-joint and joint-to-joint learning.
        """
        output = query
        intermediate = []
        intermediate_reference_points = []
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # Apply contrastive learning if joint and non-joint embeddings are provided
            if joint_embeddings is not None and non_joint_embeddings is not None:
                contrastive_loss += self.compute_contrastive_loss(joint_embeddings, non_joint_embeddings)
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), contrastive_loss

        return output, reference_points, contrastive_loss

    def compute_contrastive_loss(self, joint_embeddings, non_joint_embeddings):
        """
        Computes contrastive loss between joint embeddings (positive) and non-joint embeddings (negative).

        Args:
            joint_embeddings (Tensor): Embeddings for joints (positive samples).
            non_joint_embeddings (Tensor): Embeddings for non-joints or background (negative samples).

        Returns:
            Tensor: The computed contrastive loss.
        """
        # Positive samples (joint-to-joint) should be close
        positive_distances = F.pairwise_distance(joint_embeddings[:, None, :], joint_embeddings[None, :, :])

        # Negative samples (joint-to-non-joint) should be far
        negative_distances = F.pairwise_distance(joint_embeddings[:, None, :], non_joint_embeddings[None, :, :])

        # Contrastive loss using hinge-style formulation
        positive_loss = torch.mean(positive_distances ** 2)  # Pull joint embeddings closer
        negative_loss = torch.mean(torch.clamp(self.contrastive_margin - negative_distances, min=0) ** 2)  # Push non-joint embeddings apart
